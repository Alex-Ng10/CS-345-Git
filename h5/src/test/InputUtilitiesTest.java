...
class InputUtilitiesTest
{
  ...
  {
    BufferedReader[] nullCohort = { null, null, null };  // All null readers
    CompositeLabeledDouble cohortResult = InputUtilities.readCohort("Label2", nullCohort, filter, calc);
    assertNotNull(cohortResult, "Cohort should not be null even when all readers are null.");
    assertTrue(cohortResult.filter().isEmpty(), "Cohort should contain no histories when all readers are null.");
  }
}