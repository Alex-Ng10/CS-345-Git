 ...
 * average for the information in one or more .trn files. It also calculate the grade point average
 * for the "cohort" (i.e., all of the individuals).
 * 
...
public class TranscriptzH5
{
   ...
   * 
   * @param args
   *          The command line arguments (i.e., the names of the .trn files)
   */
  public static void main(final String[] args)
  {
    ...
    for (int i = 0; i < args.length; i++)
    {
      ...
      try (BufferedReader in = new BufferedReader(new FileReader(args[i])))
      {
        CompositeLabeledDouble all = InputUtilities.readGradeHistory("Quality Points", in, null,
            wtc);
        in.close();
        System.out.println(all.toString(true));
      ...
      try (BufferedReader in = new BufferedReader(new FileReader(args[i])))
      {
        ...
        CompositeLabeledDouble passed = InputUtilities.readGradeHistory("Passed", in, passingFilter,
            tc);
        try
        {
   ...
   * @param calculator
   *          The Calculator for the result
   * @param grades
   *          The grade composite to use to construct the credit composite
   * @return A composite hours information
   * @throws SizeException
   *           if data in the grade History is null
   */
  private static CompositeLabeledDouble createHoursHistory(final String label, final Filter filter,
      final Calculator calculator, final CompositeLabeledDouble grades) throws SizeException
  {
    CompositeLabeledDouble result = new CompositeLabeledDouble(label, filter, calculator);
    JMUCourseTable courseTable = new JMUCourseTable();
    List<LabeledDouble> temp = grades.filter();
    for (LabeledDouble d : temp)
    {
      LabeledDouble element = new LeafLabeledDouble(d.getLabel(), courseTable.get(d.getLabel()));
      result.add(element);
    }
    return result;
  }
